...
def register_api(app, view, endpoint, url, pk, pk_type='int'):
    ...
    app.add_url_rule('/api/{}/<username>/'.format(url), defaults={pk: None},
                     view_func=view_func, methods=['GET', ])
    app.add_url_rule('/api/{}/<username>/'.format(url), view_func=view_func, methods=['POST', ])
    app.add_url_rule('/api/{}/<username>/<{}:{}>/'.format(url, pk_type, pk), view_func=view_func,
...
def init_app(app):
    ...
    app.add_url_rule('/api/users/<username>/', view_func=user_view, methods=['GET', 'PUT', 'DELETE'])
    ...
    app.add_url_rule('/api/items/<username>/<int:transaction_id>/', defaults={'item_id': None},
                     view_func=transaction_item_view, methods=['GET', ])
    app.add_url_rule('/api/items/<username>/<int:transaction_id>/', view_func=transaction_item_view, methods=['POST', ])
    app.add_url_rule('/api/items/<username>/<int:transaction_id>/<int:item_id>/', view_func=transaction_item_view,
                     methods=['GET', 'PUT', 'DELETE'])
