...
def register_api(app, view, endpoint, url, pk, pk_type='int'):
    ...
    app.add_url_rule('/api/{}/<username>/'.format(url),
                     view_func=view_func, methods=['POST', ])
    app.add_url_rule('/api/{}/<username>/<{}:{}>/'.format(url, pk_type, pk),
...
def init_app(app):
    ...
    register_api(app, TransactionAPI, 'transaction_api',
                 'transactions', 'transaction_id')
    transaction_item_view = TransactionItemAPI.as_view('transaction_item_api')

    app.add_url_rule(
        '/api/items/<username>/<int:transaction_id>/',
        defaults={'item_id': None},
        view_func=transaction_item_view,
        methods=['GET', ])
    app.add_url_rule(
        '/api/items/<username>/<int:transaction_id>/',
        view_func=transaction_item_view,
        methods=['POST', ])
    app.add_url_rule(
        '/api/items/<username>/<int:transaction_id>/<int:item_id>/',
        view_func=transaction_item_view,
